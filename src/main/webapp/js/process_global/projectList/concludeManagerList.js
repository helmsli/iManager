...
//需要修改项目申报书
var ACTION_CHANGE_APPLY = STATUS_NEED_CHANGE;
//提交结项材料
...
App.controller('myDataController', ['$scope', function($scope) {
	...
	$scope.showOwner=function(status,data){
		...
		if(isOwner())
		{
			if(status==STATUS_NEED_CHANGE)
			{
				...
				data.haveConcludeFile=false;
				return true;
			}
			else if(status == STATUS_AGREED)
			{
				data.isManager=false;
				data.haveConcludeFile=false;
				//data.action=ACTION_CONCLUDE_APPLY;
				data.ownerTaskName="结项";
				return true;
			}
			else if(status==STATUS_FINISHED_CONCLUDE)
			{
				data.isManager=false;
				data.ownerTaskName="已结项";
				return false;
				
			}
			else if(status==STATUS_NEED_CHANGE_CONCLUDE)
			{
				data.isManager=false;
				data.ownerTaskName="结项修改";
				return true;
			}
			else if(STATUS_REGECTED_CONCLUDE==status)
			{
				data.isManager=false;
				data.ownerTaskName="结项修改";
				return true;
	...
	$scope.openApplyDetail = function(dataId,status,data){
		...
		var parm=parseQueryString();
		var projectName =parm.projectName||"";
		var action = APP_NAME +"_"+data.action;
		//var status = "apply";
		if(isOwner())
		{
	    ...
	    {
			if(STATUS_APPLY_CONCLUDE==status)
			{
		...
		var contextData = {"projectName":data.projectName,"companyName":data.data3};
		
		localStorage.setItem("data",JSON.stringify(contextData));
		
		if(action=="modify"){
			url="projectManager/applicationApprove.html?projectName="+projectName + "&state="+action+"&dataId="+dataId+"&from="+from;
		}else{
			url="projectManager/applicationApprove.html?projectName="+projectName + "&state="+action+"&dataId="+dataId+"&from="+from;
		}
		location.href=url;
		 ...
		 for(var i in dataLists)
		{
			 try{
				 ...
				 if(dataLists[i].data6!="")
				{
				 ...
				 if(dataLists[i].data9!="")
					{
						 try {
							var concludeInfo = JSON.parse(dataLists[i].data9);
							dataLists[i].concludeInfo = concludeInfo;
							var applyFile = JSON.parse(concludeInfo.applyFile)

							dataLists[i].concludeApplyFile = applyFile[0];
						} catch (e) {
				 ...
				 {
					 dataLists[i].statusName="提交结项材料";
				 }
				 else if(STATUS_APPLY_CONCLUDE==dataLists[i].status)
				 {
					 dataLists[i].statusName="结项审批中";
				  }
				 else if(STATUS_FINISHED_CONCLUDE==dataLists[i].status)
				 {
					 dataLists[i].statusName="项目已结项";
				  }
				 else if(STATUS_NEED_CHANGE_CONCLUDE==dataLists[i].status)
				 {
					 dataLists[i].statusName="结项材料需要修改";
				  }else if(STATUS_REGECTED_CONCLUDE==dataLists[i].status)
					 {
						 dataLists[i].statusName="拒绝结项";
					  }
				 else
				 {
					 dataLists[i].statusName="等待审批";
