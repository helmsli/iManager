...
{  
	document.getElementById("approveTitle").innerHTML="结项材料修改";
	document.getElementById("btn_modifyapply").innerHTML="修改结项材料";
	document.getElementById("btn_childsubmit").innerHTML="修改结项材料";
	gueryApprovalResult();
  	...
  	ajaxApprove(obj,function(data){
		...
		if(data.result == 0){
			...
			var commonbiz = data.responseInfo.commonBiz;
			 ...
			 else if(STATUS_FINISHED_CONCLUDE==commonbiz.status)
			 {
				 commonbiz.approveResult="项目已结项";
			  }
			 else if(STATUS_NEED_CHANGE_CONCLUDE==commonbiz.status)
			 {
				 commonbiz.approveResult="结项材料需要修改";
			  }else if(STATUS_REGECTED_CONCLUDE==commonbiz.status)
				 {
				  commonbiz.approveResult="拒绝结项";
				  }
			 else
			 {
				 commonbiz.approveResult="等待审批";
...
function openSubmitConclude()
{
	var url="";
	var parm=parseQueryString();
	var projectName =parm.projectName||"";
	var action = APP_NAME +"_"+ STATUS_AGREED;
	
	url="applicationApprove.html?projectName="+projectName + "&state="+action+"&dataId="+parm.dataId+"&from="+parm.from;
